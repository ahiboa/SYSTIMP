...
$message=NULL;
 if (empty($_POST['client_name'])){
  $client_name=NULL;
  $message.='<p>You forgot to select a client!';
 }else
...
if(!mysqli_query($dbc, "insert into event (venue, event_date_time, theme, centerpiece, flowers, linencolor,
        ...
        values ('{$venue}', '{$event_date_time}', '{$theme}', '{$centerpiece}', '{$flowers}', '{$linen}', '{$others}', 
                '{$totalpax}', '{$eventtype}', '{$package_id}', '{$event_name}', '{$client_name}', 'Approved')")){
...
$query5="select client_name, client_id from client_ref";
...
<p>Date: <input type="text" name="event_date_time" size="20" maxlength="30" value="<?php if (isset($_POST['$event_date_time']) && !$flag) echo $_POST['$event_date_time']; ?>"/>
